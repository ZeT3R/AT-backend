...
def dop(A, bits=bit_depth):
    A = rev(A, bits)
    if A[0] == '1':
        A = addition(A, ['1'], bits=len(A), code='dop')

    return A


def addition(A, B, bits=bit_depth, code='str', kr=2):
    if kr == 2:
        while len(B) != bits:
            B.insert(0, "0")
    else:
        # if B[0] == '0':
        #     while len(B) != bits + 1 and len(B) < bits + 1:
        #         B.insert(0, "0")
        # else:
        while len(B) != len(A):
            B.append("0")
    ...
    for i in range(len(A) - 1, -1, -1):
        if i == 0 and ((flag == True) and ((A[i] == '1' and B[i] == '0') or
            ...
            if kr == 3:
                if code == "rev":
                    C = addition(C, ['1'], code='rev', kr=2, bits=len(C))
                if A[i] == '1' and B[i] == '1':
                    C[i] = '1'
                    continue
                else:
                    continue
            C = addition(C, ['1'], code='str', kr=kr)
